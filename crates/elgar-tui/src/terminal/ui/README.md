...
- `approval_action.rs` defines the selected approval button action for the
  inline prompt.
- `approval_card.rs` renders the boxed approval card, selected approval action,
...
- `sections.rs` detects simple assistant response sections for display.
- `section_render.rs` renders those sections inside quiet selectable terminal
  containers.
...
- `code_tokens/` contains language normalization and token scanner helpers for
  code body styling.
- `execution_result.rs` renders raw verified execution proof as compact
