    ...
    P: ControllerProvider,
{
    pub fn turn(
        ...
        log::info!(
            "agent_turn_start session={} turn={} policy={:?} input_chars={}",
            session.id,
            session.current_turn_index().saturating_add(1),
            policy_mode,
            input.chars().count()
        );
        let result = run_agent_turn_with_policy(&self.provider, session, input, policy_mode);
        log::info!(
            "agent_turn_finish session={} route={:?} events={} actions={}",
            session.id,
            result.route,
            result.events.len(),
            session.actions().len()
    ...
    pub fn tool_turn(
        ...
        log::info!(
            "agent_tool_turn_start session={} turn={} policy={:?} input_chars={}",
            session.id,
            session.current_turn_index().saturating_add(1),
            policy_mode,
            input.chars().count()
        );
        let result = run_agent_tool_turn_with_policy(&self.provider, session, input, policy_mode);
        log::info!(
            "agent_tool_turn_finish session={} route={:?} events={} actions={}",
            session.id,
            result.route,
            result.events.len(),
            session.actions().len()
...
mod tests {
    ...
    fn agent_runtime_policy_allows_read_only_shell_commands_without_review() {
        for mode in [
            ...
            let runtime = shell_runtime("find . -maxdepth 2 -type f | head -80");
            let mut session = Session::new("session-1", &root, &root);

            let result = runtime.tool_turn(&mut session, "inspect project files", mode);
