//! Direct LM Studio smoke-test command.
//!
//! This bypasses normal sessions/TUI and sends one provider request. It is only
//! for checking that LM Studio config and connectivity work.
...
/// Converts terminal args into the prompt sent by `elgar provider-smoke`.
pub fn provider_smoke_prompt(args: &[String]) -> String {
    normalize_prompt(args.join(" "))
}

/// Reads smoke-test config from environment variables.
...
/// Builds smoke-test config from explicit values.
...
/// Runs the smoke test from environment configuration.
...
/// Sends the direct LM Studio smoke-test request.
