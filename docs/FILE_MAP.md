...
- `.elgar/` stores repo-local runtime logs.
- `~/.elgar/config/` can store user-level fallback config such as
  `elgar-provider.json` and `elgar-mcp.json`.
...
- `crates/elgar-core/src/runtime_home.rs` resolves user-level Elgar config
  paths without moving repo-local runtime logs.
...
- `~/.elgar/config/` is user-level config and is not committed.
