//! Legacy command and input tests.
...
fn terminal_commands_are_slash_only() {
    ...
    assert_eq!(
        parse_terminal_command("/reasoning"),
    ...
    assert_eq!(
        parse_terminal_command("reasoning"),
        TerminalCommand::Text("reasoning")
    );
    assert_eq!(
        parse_terminal_command("trace"),
        TerminalCommand::Text("trace")
    );
...
#[test]
fn terminal_empty_raw_command_is_local_usage() {
    let controller = Controller::default();
    let root = temp_root("terminal-empty-raw-local");
    let mut session = Session::new("session-1", root.clone(), root.clone());
    let mut shell = TuiShell::new();
    let mut pending_turn = None;
