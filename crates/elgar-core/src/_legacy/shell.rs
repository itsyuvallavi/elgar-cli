...
fn normalized_exit_code(
    ...
    if shell_exit_code != Some(0) || !command.contains("EXIT_CODE=$?") {
    ...
    stdout
        ...
        .chain(stderr.lines())
        .rev()
        .find_map(exit_code_marker_value)
        .or(shell_exit_code)
}

fn exit_code_marker_value(line: &str) -> Option<i32> {
    let marker = line.trim().strip_prefix("EXIT_CODE=")?;
    marker.trim().parse::<i32>().ok()
}
...
mod tests {
    ...
    #[test]
    fn wrapper_exit_code_marker_preserves_underlying_status() {
        let root = root("wrapped-exit-code");
        let action = ShellCommandAction::new("false 2>&1; echo \"EXIT_CODE=$?\"", root);

        let result = execute_shell_command(&action).unwrap();

        assert_eq!(result.stdout, "EXIT_CODE=1\n");
