...
## Latest Live Trace Summary

After a live TUI/provider smoke, run:

```sh
./bin/perf-trace
```

It does not call the provider. It reads the newest local trace summary and
prints provider round count, request modes, tool counts, action count, provider
duration, first chunk latency when available, token usage, request bytes,
visible output chars, and thinking chars.
