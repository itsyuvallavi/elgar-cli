...
Risky primitives require explicit user approval before execution unless the
current permission mode explicitly allows the request. Elgar will return
verified permission evidence when approval is needed."#
    )
}
